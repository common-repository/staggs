...
?>
<div class="option-group-options options cards option-group-options--<?php echo esc_attr( $columns ); ?><?php echo esc_attr( $classes ); ?>"<?php echo wp_kses_normalize_entities(  $attributes ); ?>>
	<?php
	foreach ( $sanitized_step['options'] as $key => $option ) {
		...
		if ( isset( $option['note'] ) && $option['note'] ) {
			$option_note = '<small class="option-note">' . $option['note'] . '</small>';
		}
		...
		if ( isset( $option['manage_stock'] ) && 'yes' === $option['manage_stock'] && 0 >= $option['stock_qty'] ) {
			...
			$attributes = apply_filters( 'staggs_cards_item_attributes', $attributes, $sanitized_step, $option, $key );
